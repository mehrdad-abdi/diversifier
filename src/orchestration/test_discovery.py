...
class TestCoverage:
    ...
    confidence_score: float = 0.0  # 0.0 to 1.0, how confident we are the test covers this usage
...
class TestDiscoveryAnalyzer:
    ...
    async def discover_test_coverage(
        ...
        """
        self.logger.info(f"Discovering test coverage for {library_usage.total_usages} library usages")
        ...
        self.logger.info(f"Found {len(all_test_functions)} test functions in {len(test_files)} test files")
        ...
        uncovered_usages = [
            usage for usage in library_usage.usage_locations
        ...
        coverage_percentage = (
            ...
            if library_usage.usage_locations else 0
        )
        
        # Filter relevant tests (tests that actually cover some library usage)
        relevant_tests = list({
            test for coverage in coverage_analysis 
        ...
        for pattern in test_patterns:
            try:
                if self.mcp_manager.is_server_available(MCPServerType.FILESYSTEM):
                    ...
                    test_files.update(str(p) for p in self.project_root.rglob(pattern.replace("**/", "")) if p.is_file())
                    
            except Exception as e:
                self.logger.warning(f"Error finding test files with pattern {pattern}: {e}")
                
        return list(test_files)
        
    async def _parse_test_functions(self, test_file: str, target_library: str) -> List[TestFunction]:
        ...
        try:
            ...
            if self._imports_target_library(imports, target_library):
                self.logger.debug(f"Skipping {test_file} - directly imports {target_library}")
                return []
            ...
            class TestFunctionVisitor(ast.NodeVisitor):
                ...
                def visit_FunctionDef(self, node: ast.FunctionDef):
                    if self._is_test_function(node.name):
                        test_function = TestFunction(
                            ...
                            source_code=self._get_function_source(node, content),
                            imports=imports,
                ...
                def _get_function_source(self, node: ast.FunctionDef, file_content: str) -> str:
    ...
    async def _analyze_test_coverage(
        ...
        for usage in library_usage.usage_locations:
            ...
            for test_func in test_functions:
                confidence = self._calculate_coverage_confidence(usage, test_func, target_library)
                
                if confidence > 0.3:  # Threshold for considering a test as covering the usage
                    covering_tests.append(test_func)
                    
            if covering_tests:
                # Sort by confidence (we'll implement a simple heuristic here)
                covering_tests.sort(key=lambda t: self._calculate_coverage_confidence(usage, t, target_library), reverse=True)
                
                coverage = TestCoverage(
                    usage_location=usage,
                    covering_tests=covering_tests,
                    confidence_score=self._calculate_coverage_confidence(usage, covering_tests[0], target_library)
                )
    ...
    def _calculate_coverage_confidence(
        ...
        if test_func.file_path.replace('/test_', '/').replace('_test.py', '.py') == usage.file_path:
            confidence += 0.4
        elif Path(test_func.file_path).stem.replace('test_', '').replace('_test', '') in usage.file_path:
            confidence += 0.3
