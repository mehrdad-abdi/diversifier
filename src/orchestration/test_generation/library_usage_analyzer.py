...
class LibraryUsageAnalyzer:
    ...
    async def analyze_library_usage(self, target_library: str) -> LibraryUsageSummary:
            ...
            try:
                ...
                if file_usage:
                    ...
                    for usage in file_usage:
                        if usage.usage_type in [LibraryUsageType.IMPORT, LibraryUsageType.FROM_IMPORT]:
        ...
        self.logger.info(f"Found {summary.total_usages} usages of {target_library} in {len(affected_files)} files")
        return summary
        
    def _analyze_file_usage(self, tree: ast.AST, target_library: str, file_path: str) -> List[LibraryUsageLocation]:
        ...
        class LibraryUsageVisitor(ast.NodeVisitor):
            ...
            def visit_Import(self, node: ast.Import):
                for alias in node.names:
                    if alias.name == target_library or alias.name.startswith(f"{target_library}."):
                        usage_locations.append(LibraryUsageLocation(
                            file_path=file_path,
            ...
            def visit_ImportFrom(self, node: ast.ImportFrom):
                if node.module and (node.module == target_library or node.module.startswith(f"{target_library}.")):
                    for alias in node.names:
                        usage_locations.append(LibraryUsageLocation(
                            file_path=file_path,
                            line_number=node.lineno,
            ...
            def _is_library_attribute_usage(self, node: ast.Attribute) -> bool:
                ...
                if isinstance(node.value, ast.Name):
                    ...
                    return self._get_full_attribute_name(node).startswith(target_library)
                return False
    ...
    def get_affected_functions(self, usage_summary: LibraryUsageSummary) -> Dict[str, List[LibraryUsageLocation]]:
        ...
        for usage in usage_summary.usage_locations:
            if usage.function_name:
                ...
                if usage.class_name:
                    key = f"{usage.file_path}::{usage.class_name}::{usage.function_name}"
    ...
    def get_usage_statistics(self, usage_summary: LibraryUsageSummary) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_used_files": sorted(usage_by_file.items(), key=lambda x: x[1], reverse=True)[:10]
        }