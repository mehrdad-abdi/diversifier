...
class EfficientTestGenerator:
    ...
    async def generate_efficient_tests(
        ...
        self.logger.info(f"Starting efficient test generation pipeline for {target_library}")
        
        try:
            # Step 1: Analyze library usage across the project
            self.logger.info("Step 1: Analyzing library usage with Python AST...")
            library_usage = await self.usage_analyzer.analyze_library_usage(target_library)
            
            if library_usage.total_usages == 0:
                self.logger.warning(f"No usage of {target_library} found in the project")
                return self._create_empty_result(target_library, time.time() - start_time)
            ...
            self.logger.info(f"Generated {focused_tests.tests_generated} new focused tests")
            ...
            return EfficientTestGenerationResult(
                ...
                output_directory=output_dir or str(self.project_root / "generated_library_tests")
            )
            
    def _create_empty_result(self, target_library: str, execution_time: float) -> EfficientTestGenerationResult:
    ...
    async def _validate_generated_tests(self, test_dir: str) -> bool:
        ...
        if not self.mcp_manager.is_server_available(MCPServerType.TESTING):
            self.logger.info("Testing MCP server not available, skipping test validation")
            return True
    ...
    def get_generation_summary(self, result: EfficientTestGenerationResult) -> Dict[str, Any]:
