...
class FocusedTestGenerator:
    ...
    async def generate_focused_tests(
        ...
        for usage in test_discovery.uncovered_usages:
            try:
                ...
                self.logger.warning(f"Failed to generate test for usage at {usage.file_path}:{usage.line_number}: {e}")
                
        # Also generate additional tests for low-confidence covered usages
        low_confidence_usages = [
            cov.usage_location for cov in test_discovery.usage_coverage
        ...
        for usage in low_confidence_usages[:5]:  # Limit to 5 additional tests
            try:
                ...
                self.logger.warning(f"Failed to generate additional test for usage at {usage.file_path}:{usage.line_number}: {e}")
                
        success_rate = len(generated_tests) / (len(test_discovery.uncovered_usages) + len(low_confidence_usages))
        
        result = TestGenerationResult(
            generated_tests=generated_tests,
            target_library=target_library,
            total_usage_points=len(test_discovery.uncovered_usages) + len(low_confidence_usages),
            tests_generated=len(generated_tests),
            generation_success_rate=success_rate
        )
        
        self.logger.info(f"Generated {len(generated_tests)} focused tests (success rate: {success_rate:.1%})")
    ...
    def _extract_function_context(self, source_content: str, usage: LibraryUsageLocation) -> str:
...
Generate the focused unit test now:"""
    ...
    def _parse_generated_test(self, response_text: str, usage: LibraryUsageLocation) -> Optional[GeneratedTest]:
        ...
        try:
            ...
            return GeneratedTest(
                test_name=test_data.get("test_name", f"test_{usage.function_name or 'usage'}"),
                test_code=test_data.get("test_code", ""),
                usage_location=usage,
                description=test_data.get("description", "Generated test for library usage"),
    ...
    async def export_generated_tests(
        ...
        for test_file_name, tests in tests_by_file.items():
            test_file_content = self._generate_test_file_content(tests, generation_result.target_library)
        ...
        self.logger.info(f"Exported {generation_result.tests_generated} tests to {output_dir}")
        
        return str(output_path)
        
    def _generate_test_file_content(self, tests: List[GeneratedTest], target_library: str) -> str:
        ...
        if any("mock" in test.test_code.lower() or "@patch" in test.test_code for test in tests):
